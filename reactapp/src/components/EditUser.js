...
function EditUser() {
  ...
  const {id}=useParams();
    ...
    const onSubmit=async (e)=>{
      e.preventDefault();
      console.log(user)
      await axios.put(`https://8080-dafcbabecacecbefdccdeaeaadbdbabf.project.examly.io/api/users/${id}`,user)
      navigate("/")
    };
 const loadUser =async ()=>{
    const result=await axios.get(`https://8080-dafcbabecacecbefdccdeaeaadbdbabf.project.examly.io/api/users/${id}`)
    setUser(result.data)
 }
